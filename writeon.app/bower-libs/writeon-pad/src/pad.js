(function(root, doc) {
  ...
  var Pad, debugMode, selection, utils = {};
  ...
  function initToolbar(ctx) {
    ...
    if (icons) {
      ...
      ctx._menu.setAttribute('class', ctx.config.class + '-menu pad-menu');
  ...
  function initEvents(ctx) {
    ...
    addListener(ctx, editor, 'keydown', function(e) {
      editor.classList.remove('pad-placeholder');
  ...
  function checkPlaceholder(ctx) {
    ctx.config.editor.classList[ctx.isEmpty() ? 'add' : 'remove']('pad-placeholder');
  }
  ...
  Pad.prototype.on = function(type, listener) {
    addListener(this, this.config.editor, type, listener);
    return this;
  };

  Pad.prototype.isEmpty = function(node) {
  ...
  Pad.prototype.getContent = function() {
    return this.isEmpty() ?  '' : trim(this.config.editor.innerHTML);
  };

  Pad.prototype.setContent = function(html) {
    this.config.editor.innerHTML = html;
    this.cleanContent();
    return this;
  };

  Pad.prototype.checkContentChange = function () {
  ...
  Pad.prototype.setRange = function(range) {
  ...
  Pad.prototype.focus = function(focusStart) {
    if (!focusStart) this.setRange();
    this.config.editor.focus();
    return this;
  };

  Pad.prototype.execCommand = function(name, value) {
  ...
  // pad.cleanContent({cleanAttrs: ['style'], cleanTags: ['id']})
  Pad.prototype.cleanContent = function(options) {
  ...
  Pad.prototype.autoLink = function() {
    autoLink(this.config.editor);
    return this.getContent();
  };

  // highlight menu
  Pad.prototype.highlight = function() {
  ...
  Pad.prototype.menu = function() {
    ...
    if (menuOffset.x < 0) {
      menuOffset.x = 0;
      stylesheet.insertRule('.pad-menu:after {left: ' + left + 'px;}', 0);
    } else {
      stylesheet.insertRule('.pad-menu:after {left: 50%; }', 0);
    }
    if (menuOffset.y < 0) {
      menu.classList.add('pad-menu-below');
      menuOffset.y = offset.top + offset.height + menuPadding;
    } else {
      menu.classList.remove('pad-menu-below');
    }
  ...
  Pad.prototype.stay = function(config) {
    var ctx = this;
  ...
  Pad.prototype.destroy = function(isAJoke) {
  ...
  Pad.prototype.rebuild = function() {
    return this.destroy('it\'s a joke');
  };

  // a fallback for old browers
  root.Pad = function(config) {
    if (!config) return utils.log('can\'t find config', true);

    var defaults = utils.merge(config)
      , klass = defaults.editor.getAttribute('class');

    klass = klass ? klass.replace(/\bpad\b/g, '') + ' pad-textarea ' + defaults.class : 'pad pad-textarea';
